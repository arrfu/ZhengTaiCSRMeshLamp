...
#pragma mark - Private
+ (AFHTTPSessionManager *)LxShareManager{
    static dispatch_once_t onceToken;
    dispatch_once(&onceToken, ^{
...
//            }
            manager = [HYBNetworking LxShareManager];
            ...
            //  allowInvalidCertificates代表是否允许不信任的证书通过验证，默认为NO
            
            defaultPolicy.allowInvalidCertificates = YES;
            
            // validatesDomainName属性代表是否验证主机名，默认为YES。
            
            defaultPolicy.validatesDomainName = NO;
            
            manager.securityPolicy = defaultPolicy;
